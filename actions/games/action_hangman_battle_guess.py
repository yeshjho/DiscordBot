...
class ActionHangmanBattleGuess(Action):
    async def on_message(self, msg: Message, **kwargs):
        from db_models.hangman.models import HangmanBattleSession, HangmanGame, EHangmanBattleState, \
        ...
        if guess_result == EHangmanBattleGuessResult.WRONG:
            ...
            if session.user1_wrong_count == session.user2_wrong_count:
                ...
                await self.send_result_msg(msg, session, result)
        ...
        txt, embed = game.get_msg(user1_nick, user2_nick, EHangmanBattleState(session.state))
        try:
            game_msg = await msg.channel.fetch_message(session.msg_id)
        except nextcord.errors.NotFound:
            msg_id = (await msg.channel.send(txt, embed=embed)).id
            session.msg_id = msg_id
            session.save()
        else:
            await game_msg.edit(content=txt, embed=embed)

        return author_id, "guessed letter", c, "in battle"

    async def send_result_msg(self, msg, session, result):
        ...
        if result == HangmanBattleGame.EResult.PLAYER_1_WIN:
            embed.add_field(name='승리', value=mention_user(session.game.user1.id))
            embed.add_field(name='패배', value=mention_user(session.game.user2.id))
        elif result == HangmanBattleGame.EResult.PLAYER_2_WIN:
            embed.add_field(name='승리', value=mention_user(session.game.user2.id))
            embed.add_field(name='패배', value=mention_user(session.game.user1.id))
        else:
            embed.add_field(name='무승부', value=EMPTY_LETTER)
